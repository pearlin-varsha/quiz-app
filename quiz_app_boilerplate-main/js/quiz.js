...
let questions =[
    ...
    //try to convert everything as one single object
    let value = JSON.stringify(questions)
    //local storage - permanent storage, session storage - only for particular time interval
    ...
    //function show defnition
    function show(count){
    let question = document.getElementById("questions")
    let [first,second,third,fourth] = questions[count].options
    //$ directly modify inside html
    question.innerHTML= `
    ...
    //fuction next
    function next(){
    //to check wheather it is last ques or not
